...
def main():
    ...
    model = Net(sensing_rate=args.sensing_rate, LayerNo=args.layer_num)
    model = nn.DataParallel(model)
    model = model.to(device)

    model_dir = "./%s/%s_layer_%d_lr_%.4f_ratio_%.2f" % (args.save_dir, args.model, args.layer_num, args.lr, args.sensing_rate)
    checkpoint = torch.load("%s/net_params_best.pth" % (model_dir), map_location=device)
    model.load_state_dict(checkpoint['net'])
    epochs = checkpoint['epoch']

    test_dir = os.path.join("F:\dataset\PNG\RGB", args.test_name)
    ...
    print('\n')
    output_data = "CS ratio is %.2f, Avg PSNR/SSIM/LPIPS/Time for %s is %.2f/%.4f/%.4f/%.4f, Epoch number of model is %d \n" % (
        args.sensing_rate, args.test_name, np.mean(PSNR_All), np.mean(SSIM_All), np.mean(LPIPS_All),np.mean(Time_All), epochs)
    print(output_data)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_dir', type=str, default='save_model', help='The directory used to save models')
    parser.add_argument('--layer_num', type=int, default=10, help='phase number of the Net')
    parser.add_argument('--test_name', type=str, default='McM18', help='name of test set')
    parser.add_argument('--result_dir', type=str, default='recon_img', help='result directory')
    parser.add_argument('--lr', '--learning_rate', default=3e-4, type=float, help='initial learning rate')
    main()
